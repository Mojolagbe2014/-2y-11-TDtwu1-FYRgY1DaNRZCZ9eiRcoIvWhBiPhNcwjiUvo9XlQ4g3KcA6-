...
class Contest implements ContentManipulator{
     ...
     */
    function add(){
        $sql = "INSERT INTO ".self::$tableName." (`title`, `intro`, `description`, `header`, `logo`, `start_date`, `end_date`, `announcement_date`, `winners`, `question`, `answer`, `point`, `bonus_point`, `rules`, `prize`, `message`, `css`, `status`, `date_added`) "
                ."VALUES ('{$this->title}','{$this->intro}','{$this->description}','{$this->header}','{$this->logo}','{$this->startDate}','{$this->endDate}','{$this->announcementDate}','{$this->winners}','{$this->question}','{$this->answer}','{$this->point}','{$this->bonusPoint}','{$this->rules}','{$this->prize}','{$this->message}','{$this->css}','{$this->status}',$this->dateAdded)";
    ...
    public function fetchRaw($column="*", $condition="", $sort="id"){
        $sql = "SELECT $column FROM ".self::$tableName." ORDER BY $sort";
        if(!empty($condition)){$sql = "SELECT $column FROM contest WHERE $condition ORDER BY $sort";}
    ...
    public function update() {
        $sql = "UPDATE ".self::$tableName." SET title = '{$this->title}', intro = '{$this->intro}', description = '{$this->description}', header = '{$this->header}', logo = '{$this->logo}', start_date = '{$this->startDate}', end_date = '{$this->endDate}', announcement_date = '{$this->announcementDate}', winners = '{$this->winners}', question = '{$this->question}', answer = '{$this->answer}', point = '{$this->point}', bonus_point = '{$this->bonusPoint}', rules = '{$this->rules}', prize = '{$this->prize}', message = '{$this->message}', css = '{$this->css}' WHERE id = $this->id ";
        if(!empty($this->id)){
            $result = self::$dbObj->query($sql);
