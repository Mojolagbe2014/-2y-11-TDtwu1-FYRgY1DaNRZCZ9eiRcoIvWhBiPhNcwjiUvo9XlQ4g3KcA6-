...
class Admin implements ContentManipulator{
     ...
     */
    function add(){
        $sql = "INSERT INTO ".self::$tableName." (name, email, username, password, role, date_registered) "
        ...
        if($this->notEmpty($this->name,$this->email,$this->role)){
            ...
            else{ $json = array("status" => 2, "msg" => "Error adding admin! ".  mysqli_error(self::$dbObj->connection)); }
        }
    ...
    public function delete(){
        ...
        if($this->notEmpty($this->id)){
            ...
            else{ $json = array("status" => 2, "msg" => "Error deleting admin! ".  mysqli_error(self::$dbObj->connection));  }
        }
    ...
    public function update() {
        $sql = "UPDATE ".self::$tableName." SET name = '{$this->name}', email = '{$this->email}', username = '{$this->userName}', role = '{$this->role}' WHERE id = $this->id ";
        if(!empty($this->id)){
            $result = self::$dbObj->query($sql);
            if($result !== false){ $json = array("status" => 1, "msg" => "Done, admin successfully update!"); }
            else{ $json = array("status" => 2, "msg" => "Error updating admin! ".  mysqli_error(self::$dbObj->connection));   }
        }
        else{ $json = array("status" => 3, "msg" => "Request method not accepted."); }
        self::$dbObj->close();
        header('Content-type: application/json');
    ...
    public function changePassword($newPassword){
        ...
        if($pwdExists==TRUE){
            ...
            else{ $json = array("status" => 2, "msg" => "Error updating admin password! ".  mysqli_error(self::$dbObj->connection));   }
        }
